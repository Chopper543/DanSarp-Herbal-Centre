...
-- Includes: phone_verified (migration 020), two_factor_secret, two_factor_backup_codes (migration 021)
CREATE TABLE users (
  id UUID PRIMARY KEY REFERENCES auth.users(id) ON DELETE CASCADE,
  email TEXT NOT NULL,
  full_name TEXT,
...
-- Insert blog posts (from migration 019)
-- Note: This is a large data migration. The blog posts will be inserted
-- when an admin user exists in the database.
-- The full blog posts content is in migration 019_insert_ghanaian_health_blog_posts.sql
-- For the consolidated file, we'll include a note that blog posts should be inserted
-- after the first admin user is created, or include the full blog posts if needed.
...
-- 
-- NOTES:
-- 1. Admin promotions (migrations 004 and 012) are disabled - promote admins manually
-- 2. Blog posts (migration 019) - Full blog posts insertion should be done separately
--    or included here if needed. The migration file contains 30 blog posts.
-- 3. Storage bucket "avatars" must be created manually in Supabase Dashboard
-- 4. This consolidated file represents the final state after all 21 migrations
-- ============================================================================
...
BEGIN
  ...
  -- If still no user found, skip blog posts insertion
  -- Blog posts can be inserted later once users are created
  IF admin_user_id IS NULL THEN
    RAISE NOTICE 'No users found in database. Skipping blog posts insertion. Blog posts can be added later once users are created.';
    RETURN;
  END IF;
