...
- [x] **HIGH — `clinical-notes/[id]` DELETE HARD-deletes a clinical record, and it is the path the admin UI calls — RESOLVED (`fix/clinical-notes-soft-delete-id`).** **H**
  - `clinical-notes/[id]/route.ts` DELETE did a physical `supabase.from("clinical_notes").delete().eq("id", id)` with **no audit event**, and the admin list UI calls exactly this (`admin/clinical-notes/page.tsx:81-82`, `method: "DELETE"` to `/api/clinical-notes/${id}`). Meanwhile the **soft-delete** collection DELETE (sets `deleted_at`/`deleted_by`/`deleted_reason` for retention) had **no caller**. Net effect: admin "delete note" **permanently destroyed** a clinical record with no trail, bypassing the HIPAA retention / soft-delete model the schema was built for (`final_schema.sql:1663-1665` — "never physically removed by the application path"). Decision: option (a) — the mirror of the §5 PUT fix. Kept the UI on `/[id]` and made that handler soft-delete, **reusing the collection path's proven logic verbatim**: existence check (404), already-deleted guard (409), `UPDATE deleted_at=now()/deleted_by=user.id/deleted_reason=sanitizeText(?reason)`, `logAuditEvent(action='delete_clinical_note', soft_delete:true, old/new)`, admin-only preserved. Then **removed the now-dead collection DELETE** (re-confirmed zero callers — the only clinical-notes `method:"DELETE"` is the admin list → `/[id]`; `SOAPNoteEditor.tsx`'s DELETE targets `/api/storage/upload`, not the notes collection) and its unused `ClinicalNoteDeleteSchema`, collapsing to a single soft-delete path. **Read-path completeness (bundled — the half that's easy to miss):** `[id]` GET and `search` did **not** filter `deleted_at`, so a soft-deleted note was still served by direct link and still surfaced in search (the list route already filtered — `route.ts:159-161`). Both now apply `.is("deleted_at", null)` with an `include_history=true` escape hatch matching the list convention, so a soft-deleted note is hidden by default but **retrievable for audit/legal review** — deleted-from-view but not gone. Coverage: `__tests__/security-clinical-notes-soft-delete-route.test.ts` (soft-delete UPDATE not `.delete()`, audit fires, 409/404, collection no longer exports DELETE) and `__tests__/security-clinical-notes-soft-delete-readpath.test.ts` (hidden-by-default + include_history escape hatch on both `[id]` GET and search). Physical delete at the DB layer is still permitted by RLS `notes_delete_admin` — tracked as its own hardening item below.
- [x] **DB-layer `BEFORE DELETE` trigger backstop — clinical records cannot be physically destroyed even by RLS-bypassing writers — RESOLVED (`fix/clinical-block-delete-trigger`).** **M**
  - `fix/clinical-notes-soft-delete-id` moved the application delete path to soft-delete, but that is an APP-layer guard only. The RLS policy `notes_delete_admin ON clinical_notes FOR DELETE USING (is_super_admin_or_admin())` (`final_schema.sql:1084`) still **permitted a physical DELETE** by any admin, and a direct table-owner / service-role writer bypasses RLS entirely — there was no `BEFORE DELETE` trigger, so nothing bound a bypassing writer to the retention model. Fixed with a trigger-based backstop (migration `20260711000001_clinical_block_delete.sql`, synced into `final_schema.sql`): `clinical_records_block_delete()` `BEFORE DELETE` on `clinical_notes` + `prescriptions` `RAISE`s (`check_violation`) unless the transaction-local GUC `app.allow_clinical_purge='on'` is set — the DELETE analog of `clinical_records_freeze_identity()` from `20260708000001` (same "triggers bind owner/service-role, RLS does not" rationale). **The escape hatch was forced, not optional:** the live right-to-be-forgotten cron (`app/api/cron/patient-data-deletions/route.ts:47-49`) physically deletes all three clinical tables via the service-role client, so a blanket block would break erasure. So the cron was rewired to call a gated `purge_patient_clinical_data(p_user_id)` `SECURITY DEFINER` RPC that (1) verifies a `pending`/`processing` `deletion_requests` row exists for the user — the cron sets `processing` before purging, so a pending-only guard would have broken every real call — else `RAISE`s; (2) sets the tx-local flag; (3) deletes clinical_notes/prescriptions/lab_results by `patient_id`. `REVOKE`d from public/anon/authenticated, `GRANT EXECUTE` to `service_role` only. A raw supabase-js `.delete()` can't set the flag (PostgREST is one statement per request), so the block holds against the bypassing-writer surface. Proven on the RLS-bypassing owner connection by `__tests__/security-clinical-block-delete.dbtest.ts`: physical DELETE rejected on both tables; the RPC purges end-to-end THROUGH the block when a request exists (erasure still works); the RPC `RAISE`s with no live request (hatch gated); the tx-local flag does not leak (a raw DELETE after a purge in the same session is still blocked); and the DELETE-only block leaves the soft-delete `UPDATE` untouched. Trigger + RPC + cron rewire landed as **one atomic unit** (splitting them would break the erasure pipeline).
  - **PREREQUISITE FOR lab_results parity (deferred — its own branch):** `lab_results` was deliberately left OUT of the block trigger. Unlike clinical_notes (soft-delete path already shipped) and prescriptions (no per-id physical-delete handler), `lab_results` still has **live physical-delete admin API handlers** (`lab-results/[id]/route.ts:210`, `lab-results/route.ts:489`) and **no soft-delete columns** (`deleted_at/deleted_by/deleted_reason`). Adding the block trigger now would break those endpoints with no soft-delete path to land on. It needs the notes-style conversion FIRST — add the three soft-delete columns to `lab_results`, convert both admin DELETE handlers to soft-delete + read-path filtering (mirror `fix/clinical-notes-soft-delete-id`), THEN extend `clinical_records_block_delete()` to `lab_results` for full parity. The purge RPC already erases `lab_results` (same erasure semantics), so the erasure pipeline is unaffected by the deferral — only the physical-delete backstop is missing on that one table.
