...
- [ ] **DB-layer `BEFORE DELETE` trigger backstop — clinical records cannot be physically destroyed even by RLS-bypassing writers.** **M**
  - `fix/clinical-notes-soft-delete-id` moved the application delete path to soft-delete, but that is an APP-layer guard only. The RLS policy `notes_delete_admin ON clinical_notes FOR DELETE USING (is_super_admin_or_admin())` (`final_schema.sql:1084`) still **permits a physical DELETE** by any admin, and a direct table-owner / service-role writer bypasses RLS entirely — there is no `BEFORE DELETE` trigger, so nothing binds a bypassing writer to the retention model. Add `clinical_records_block_delete()` `BEFORE DELETE` on `clinical_notes` (and, for parity, `prescriptions` / `lab_results`) that `RAISE EXCEPTION`s — the DELETE analog of `clinical_records_freeze_identity()` from `20260708000001` (same "triggers bind owner/service-role, RLS does not" rationale). Prove on the RLS-bypassing owner connection that a physical `DELETE` is rejected while the soft-delete `UPDATE` still passes. Scopeable defense-in-depth, its own branch — deliberately NOT bundled into the soft-delete fix. (If a genuine purge path is ever needed for true legal erasure, gate it behind a distinct SECURITY DEFINER RPC, not the general DELETE.)
