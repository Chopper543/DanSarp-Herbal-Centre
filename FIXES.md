...
- [ ] **`clinical-notes/[id]` PUT updates in place instead of appending an amendment version.** **M**
  - The append-only amendment model (Phase 3, `final_schema.sql:1561`) says an edit should INSERT a new version (`amended_from_id`→parent, bump `version`, set the parent's `superseded_by_id`) and preserve the old row verbatim for the HIPAA amendment audit trail. The collection route (`clinical-notes/route.ts` PUT) does this, but `clinical-notes/[id]/route.ts` PUT still does a plain in-place `UPDATE` — so an edit through the `/[id]` path silently overwrites the prior note content with no version history, defeating the amendment model. `fix/clinical-immutable-db-trigger` extended that handler's field allowlist (identity/classification now frozen) but deliberately left the in-place-vs-amendment behaviour unchanged — that is an audit-trail-integrity fix of its own. Route the `/[id]` edit through the same amendment path (or the `amend_clinical_note` RPC) so no clinical content mutation escapes the version chain.
